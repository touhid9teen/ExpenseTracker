...
- No dev-user fallback: without DB/auth, auth returns `null` and `GET /api/expenses` returns `SEED_EXPENSES` demo data. **All routes are now protected by `src/middleware.js`** — it verifies the `auth_token` cookie (same `decrypt()` from `src/lib/jwt.js`) and redirects unauthenticated page requests to `/login` (401 JSON for APIs). Public exceptions: `/login`, `/terms`, `/api/auth/*`, `/api/init-db`, and static assets (matcher).
