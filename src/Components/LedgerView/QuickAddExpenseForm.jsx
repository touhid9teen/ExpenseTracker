    ...
    return (
        <div className={`p-6 rounded-2xl border shadow-xl transition-all duration-300 transform scale-100 origin-top ${darkMode ? "bg-slate-900 border-slate-800 shadow-black/30" : "bg-white border-slate-200 shadow-slate-300/60"}`}>
            <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm font-extrabold uppercase tracking-widest flex items-center gap-2 text-amber-500">
            ...
            <form onSubmit={handleAddExpense} className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <input type="text" required placeholder="e.g. Rice, meat and vegetables" value={addDescription} onChange={(e) => setAddDescription(e.target.value)} className={`w-full px-4 py-2.5 rounded-xl border text-xs focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-all ${darkMode ? "bg-[#1e293b] border-slate-700 text-slate-150 placeholder-slate-500" : "bg-slate-50 border-slate-300 text-slate-800 placeholder-slate-400"}`} />
                </div>
                <div>
                    ...
                    <input type="number" required min="0.01" step="0.01" placeholder="0.00" value={addAmount} onChange={(e) => setAddAmount(e.target.value)} className={`w-full px-4 py-2.5 rounded-xl border text-xs focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-all ${darkMode ? "bg-[#1e293b] border-slate-700 text-slate-150 placeholder-slate-500" : "bg-slate-50 border-slate-300 text-slate-800 placeholder-slate-400"}`} />
                </div>
                <div>
                    ...
                    <select value={addCategory} onChange={(e) => setAddCategory(e.target.value)} className={`w-full px-4 py-2.5 rounded-xl border text-xs focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-all ${darkMode ? "bg-[#1e293b] border-slate-700 text-slate-150" : "bg-slate-50 border-slate-200 text-slate-800"}`}>
                ...
                <div>
                    ...
                    <input type="date" required value={addDate} onChange={(e) => setAddDate(e.target.value)} className={`w-full px-4 py-2.5 rounded-xl border text-xs focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-all ${darkMode ? "bg-[#1e293b] border-slate-700 text-slate-150" : "bg-slate-50 border-slate-200 text-slate-800"}`} />
                </div>
                <div className="md:col-span-4 flex justify-end gap-3 mt-2 border-t pt-4 border-slate-300 dark:border-slate-800">
                    ...
                    <button type="submit" className="px-6 py-2.5 rounded-xl text-xs font-bold text-white bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 shadow-md shadow-amber-500/10 focus:outline-none">Save Expense</button>
                </div>
            </form>
        </div>
    );
