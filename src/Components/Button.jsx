...
const Button = ({
    ...
    return (
        ...
        >
            <div className={`absolute inset-0 transition-opacity duration-300 ${loading ? v.loading : v.base}`} />
            {!loading && shimmer && (
                <div className="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-700 bg-gradient-to-r from-transparent via-white/10 to-transparent" />
            )}
            {!loading && glow && (
                <div className={`absolute -inset-1 bg-gradient-to-r ${v.glow} rounded-xl blur-md opacity-0 group-hover:opacity-100 transition-opacity duration-300`} />
            )}
