...
const AppHeader = ({ darkMode, activeTab, setActiveTab, toggleTheme, user, handleLogout }) => {
    return (
            <header className={`sticky top-0 z-30 transition-colors duration-300 border-b ${darkMode ? "bg-[#0f172a]/90 border-slate-800/80 backdrop-blur-md" : "bg-white/95 border-slate-300/85 backdrop-blur-md shadow-md shadow-slate-200/30"}`}>
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 min-h-16 py-3 sm:py-0 flex flex-wrap sm:flex-nowrap items-center justify-between gap-3">
                    ...
                    <div className="hidden sm:flex order-3 w-full sm:order-none sm:w-auto items-center">
                        <div className={`w-full sm:w-auto p-1 rounded-xl grid grid-cols-3 sm:flex gap-1 border ${darkMode ? "bg-slate-900 border-slate-800" : "bg-slate-100 border-slate-300"}`}>
                            <button
                                onClick={() => setActiveTab("statistics")}
                                className={`px-3 sm:px-4 py-2 sm:py-1.5 rounded-lg text-[11px] sm:text-xs font-bold transition-all duration-200 flex items-center justify-center gap-1.5 ${activeTab === "statistics" ? (darkMode ? "bg-slate-800 text-emerald-400 shadow-sm shadow-black/5" : "bg-white text-emerald-600 shadow-sm shadow-slate-200/20") : (darkMode ? "text-slate-400 hover:text-slate-200" : "text-slate-500 hover:text-slate-850")}`}
                                id="nav-btn-stats"
                            >
                            ...
                            <button
                                ...
                                className={`px-3 sm:px-4 py-2 sm:py-1.5 rounded-lg text-[11px] sm:text-xs font-bold transition-all duration-200 flex items-center justify-center gap-1.5 ${activeTab === "ledger" ? (darkMode ? "bg-slate-800 text-emerald-400 shadow-sm shadow-black/5" : "bg-white text-emerald-600 shadow-sm shadow-slate-200/20") : (darkMode ? "text-slate-400 hover:text-slate-200" : "text-slate-500 hover:text-slate-850")}`}
                                id="nav-btn-ledger"
                            >
                            ...
                            <button
                                ...
                                className={`px-3 sm:px-4 py-2 sm:py-1.5 rounded-lg text-[11px] sm:text-xs font-bold transition-all duration-200 flex items-center justify-center gap-1.5 ${activeTab === "about" ? (darkMode ? "bg-slate-800 text-emerald-400 shadow-sm shadow-black/5" : "bg-white text-emerald-600 shadow-sm shadow-slate-200/20") : (darkMode ? "text-slate-400 hover:text-slate-200" : "text-slate-500 hover:text-slate-850")}`}
                                id="nav-btn-about"
                            >
                    ...
                    <div className="flex shrink-0 items-center gap-3">
                        ...
                        <button
                            onClick={toggleTheme}
                            className={`p-2.5 rounded-xl border transition-all duration-200 hover:scale-105 ${darkMode ? "bg-slate-800 border-slate-700/80 text-amber-400 hover:bg-slate-750" : "bg-slate-100 border-slate-300 text-slate-650 hover:bg-slate-200"}`}
                            id="theme-toggler"
