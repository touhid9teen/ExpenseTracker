...
// Dev-mode mock: a randomly generated reset code (shown in the UI). Used when
// the database is not configured OR unreachable (dev only).
const mockRecoverToken = () => {
...
// Dev-mode mock: accept any valid-looking token.
const mockResetSuccess = () => {
  return NextResponse.json({
    success: true,
...
async function postHandler(request) {
  try {
    ...
    if (!sql) return mockRecoverToken();
    ...
    try {
      ...
      return NextResponse.json({
        success: true,
        message: 'If this email is registered, you will receive a reset link.',
        ...(isDevMode ? { devToken: resetCode, devMode: true } : {}),
      });
    } catch (error) {
      // Neon unreachable — fall back to the dev mock so the flow keeps
      // working locally; fail loudly in production.
      if (isDev && isConnectionError(error)) return mockRecoverToken();
      throw error;
    }
  } catch (error) {
...
async function putHandler(request) {
  try {
    ...
    if (!sql) return mockResetSuccess();
    ...
    try {
      ...
      // Neon unreachable — fall back to the dev mock so the flow keeps
      // working locally; fail loudly in production.
      if (isDev && isConnectionError(error)) return mockResetSuccess();
      throw error;
    }
  } catch (error) {
