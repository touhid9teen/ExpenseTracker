    ...
    return (
        <div className="flex flex-col md:flex-row md:items-center justify-between border-b pb-4 border-slate-200/50 dark:border-slate-800/50">
            ...
            <div className="mt-2 md:mt-0 flex items-center gap-2">
                <span className={`px-3 py-1.5 rounded-xl text-xs font-bold border ${darkMode ? "bg-slate-900/80 border-slate-850" : "bg-slate-100 border-slate-250"}`}>Today: {safeDateLabels.today}</span>
                <button onClick={() => setActiveTab("ledger")} className="hidden sm:inline-block px-4 py-1.5 rounded-xl text-xs font-bold text-white bg-emerald-500 hover:bg-emerald-600 shadow-md shadow-emerald-500/15">Open Ledger →</button>
            </div>
        </div>
    );
};
