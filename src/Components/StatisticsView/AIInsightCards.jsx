...
export const AIInsightCards = ({ darkMode, expenses = [], setActiveTab, setPendingAction }) => {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {/* AI Insight */}
            <div className="relative overflow-hidden rounded-2xl p-6 text-white bg-gradient-to-br from-violet-500 via-purple-500 to-indigo-500 shadow-lg shadow-violet-500/30">
                ...
                <div className="relative">
                    <div className="flex items-center gap-2.5 mb-3">
                        ...
                        <h3 className="text-base font-extrabold">AI Insight</h3>
                    </div>
                    <p className="text-sm leading-relaxed text-white/90">{alertText}</p>
                    <button
                        onClick={() => askAI("Give me a detailed breakdown of my spending changes this month vs last month, by category, with the ৳ amounts.")}
                        className="mt-4 inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-bold bg-white/95 text-violet-700 transition-all duration-200 hover:scale-[1.03] active:scale-95"
                    >
                        Ask AI about this
            ...
            >
                <div className="flex items-center gap-2.5 mb-3">
                    <span className="w-9 h-9 rounded-xl bg-emerald-500/15 text-emerald-500 flex items-center justify-center">
                        <SavingsLeafIcon className="w-5 h-5" strokeWidth={2.25} />
                    </span>
                    <h3 className={`text-base font-extrabold ${darkMode ? "text-white" : "text-slate-900"}`}>
                        Savings Opportunity
                    </h3>
                </div>
                <p className={`text-sm leading-relaxed ${darkMode ? "text-slate-300" : "text-slate-600"}`}>{tipText}</p>
                <button
                    ...
                    className="mt-4 inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-bold bg-gradient-to-br from-emerald-500 to-teal-500 text-white transition-all duration-200 hover:scale-[1.03] active:scale-95"
                >
                    Get savings tips
                    <ArrowRightIcon className="w-3.5 h-3.5" strokeWidth={2.5} />
                </button>
